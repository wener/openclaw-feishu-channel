...
export function parseFeishuMessageEvent(
  ...
  // Debug: log mentions for routing analysis
  if ((event.message as any)._isRouted) {
    console.log(`[Router Debug] mentions array:`, JSON.stringify(event.message.mentions));
    console.log(`[Router Debug] botOpenId:`, botOpenId);
  }
...
export async function handleFeishuMessage(params: {
  ...
  log(`feishu[${account.accountId}]: checking dedup for message ${messageId}`);
  const isDuplicate = !(await tryRecordMessagePersistent(messageId, account.accountId, log));
  if (isDuplicate) {
    log(`feishu[${account.accountId}]: skipping duplicate message ${messageId}`);
    return;
  }
  log(`feishu[${account.accountId}]: message ${messageId} is not duplicate, continuing`);
  ...
  log(
    `feishu[${account.accountId}]: message content: "${ctx.content.substring(0, 200)}"`,
  );
  
  // Debug: log raw mentions array for comparison
  const rawMentions = event.message.mentions ?? [];
  log(`feishu[${account.accountId}]: raw mentions count: ${rawMentions.length}`);
  if (rawMentions.length > 0) {
    log(`feishu[${account.accountId}]: raw mentions: ${JSON.stringify(rawMentions.map(m => ({key: m.key, name: m.name, open_id: m.id.open_id})))}`);
  }
  ...
  if (ctx.mentionTargets && ctx.mentionTargets.length > 0) {
    ...
    log(`feishu[${account.accountId}]: no mention targets detected`);
  }
  ...
  if (isGroup) {
    ...
    // 检查是否是路由消息（通过自定义标记）
    const isRoutedMessage = (event.message as any)._isRouted === true;
    
    log(`feishu[${account.accountId}]: requireMention=${requireMention}, mentionedBot=${ctx.mentionedBot}, isRoutedMessage=${isRoutedMessage}`);
    if (requireMention && !ctx.mentionedBot && !isRoutedMessage) {
      log(
    ...
    if (isRoutedMessage) {
      log(`feishu[${account.accountId}]: routed message detected, skipping mention check`);
    }
    log(`feishu[${account.accountId}]: mention check passed, continuing to dispatch`);
  } else {
  }
  ...
  try {
    ...
    log(`feishu[${account.accountId}]: agentId=${route.agentId}`);
 ...
 * 路由消息到目标飞书账户
 * 
 * 直接调用 handleFeishuMessage，创建独立的 OpenClaw context
 * 跳过 webhook 签名、解码流程
 * 
 * @param params 路由参数
 */
export async function dispatchRoutedMessage(params: {
  cfg: ClawdbotConfig;
  targetAccountId: string;    // 目标飞书 accountId
  message: string;            // 路由消息内容
  targetName: string;         // 目标 bot 名称（如 "tech"）
  chatId: string;             // 群聊 ID
  senderOpenId: string;       // 发送者 open_id
  originalMessageId?: string; // 原始消息的真实飞书 message_id
  runtime?: RuntimeEnv;       // 运行时环境
}): Promise<void> {
  const { cfg, targetAccountId, message, targetName, chatId, senderOpenId, originalMessageId, runtime } = params;
  const log = runtime?.log ?? console.log;
  
  // 获取目标 bot 的 open_id
  const targetAccount = resolveFeishuAccount({ cfg, accountId: targetAccountId });
  const targetBotOpenId = targetAccount.botOpenId;
  
  log(`[Router] 🚀 Dispatching routed message to accountId: ${targetAccountId}, target: ${targetName}, botOpenId: ${targetBotOpenId}`);
  
  // 使用原始消息的真实 message_id，如果没有则生成一个唯一的
  const messageId = originalMessageId || `om_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  log(`[Router] Using messageId: ${messageId}`);
  
  // 构造飞书事件，添加 mentions 数组让目标 bot 认为被@了
  // 使用目标 bot 的真实 open_id
  const routedEvent = {
    message: {
      message_id: messageId,  // 使用真实的飞书 message_id
      chat_id: chatId,
      chat_type: 'group' as const,
      message_type: 'text' as const,
      content: message,  // 直接使用原始消息内容
      _isRouted: true,   // 标记为路由消息
      mentions: targetBotOpenId ? [
        {
          key: `@_user_1`,  // 使用飞书风格的占位符格式
          name: '健康-产品技术',  // 使用飞书中的 bot 显示名称
          id: {
            open_id: targetBotOpenId,  // 使用目标 bot 的真实 open_id
            user_id: targetBotOpenId,
          },
          type: 'user',
        },
      ] : [],
    ...
    sender: {
      sender_id: {
        open_id: 'ou_4329a1aa85b00ae1a751b2e186cde884',  // 使用真实用户的 open_id
        user_id: 'ou_4329a1aa85b00ae1a751b2e186cde884',
      },
    },
  };
  
  // 调用 handleFeishuMessage 处理路由消息
  await handleFeishuMessage({
    cfg,
    event: routedEvent as any,
    runtime,
    accountId: targetAccountId,
    botOpenId: targetBotOpenId,  // 传入 botOpenId 以正确识别 mentions
  });
  
  log(`[Router] ✅ Routed message dispatched to ${targetAccountId}`);
}
